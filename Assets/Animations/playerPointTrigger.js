...
function Update () {
    ...
    if(playerPos_x == transform.position.x && playerPos_y == transform.position.y){
        ...
        if(score/2 == lvl_difficultyX){
            ...
            lvl_difficultyX++;
            Debug.Log("Current lvl: "+lvl_difficultyX);
            //spawner = GameObject.Find("map");
           // spawner.GetComponent("enemySpawner").randomPosition();
            //Debug.Log( spawner.GetComponent("enemySpawner"));
            //spawner.randomPosition();
            //spawner.RandomizeArray(enemySpawner.spawnPlace);
            }
        }
}
